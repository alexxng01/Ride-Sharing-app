...
import {
  Truck, CheckCircle, XCircle, Play, MapPin,
  Navigation, AlertCircle, Clock, Banknote,
  Star, TrendingUp, CircleDot, ArrowRight, Eye, ArrowUp, ArrowDown,
  Target, Gauge, Timer
} from "lucide-react";
...
const getLocationNameFromCoords = (lat, lng) => {
  ...
  if (minDistance < 0.01 && closestMatch) {
    return closestMatch;
  }
...
function InfoRow({ label, value, accent, large }) {
  return (
    <div className="flex items-center justify-between">
      ...
      <span className={`font-semibold ${large ? "text-base" : "text-sm"} ${accent ? "text-brand-400" : "text-white"}`}>
        {value}
      </span>
    </div>
  );
...
function NavStrip({ navStats, phase }) {
  if (!navStats) return null;
  const distKm = navStats.remainingM >= 1000
    ? `${(navStats.remainingM / 1000).toFixed(1)} km`
    : `${navStats.remainingM} m`;
  const etaSec = navStats.remainingS;
  const eta = etaSec < 60 ? `${etaSec}s` : `${Math.floor(etaSec / 60)}m ${etaSec % 60}s`;

  return (
    <div className="grid grid-cols-2 gap-2">
      <div className="bg-dark-700/60 rounded-xl p-3 text-center">
        <p className="text-dark-500 text-[10px] uppercase tracking-wider mb-0.5">Remaining</p>
        <p className="text-white font-bold text-base">{distKm}</p>
      </div>
      <div className="bg-dark-700/60 rounded-xl p-3 text-center">
        <p className="text-dark-500 text-[10px] uppercase tracking-wider mb-0.5">ETA</p>
        ...
        <React.Fragment key={step.key}>
          <div className="flex flex-col items-center gap-1">
            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold border-2 transition-all ${
              ...
              i === idx ? "bg-brand-500 border-brand-500 text-white animate-pulse" :
...
function PaymentScreen({ ride, onClear }) {
    ...
    >
      <div className="bg-gradient-to-b from-green-500/10 to-dark-800/80 border border-green-500/25 rounded-2xl p-4 space-y-4">
        ...
        <div className="space-y-2.5">
          ...
          <InfoRow label="Duration" value={`${Math.floor((ride.duration || 0) / 60)}m ${(ride.duration || 0) % 60}s`} />
          ...
          <div className="border-t border-dark-700 pt-2">
            ...
            <InfoRow label="Per km" value={`NPR ${Math.round((ride.fare - 50) / Math.max(ride.distance, 1))}/km`} />
          </div>
        </div>
        ...
        <div className="text-center space-y-1.5">
          ...
          <div className="flex justify-center gap-2">
            {[1,2,3,4,5].map((n) => (
              <button key={n} className="text-2xl hover:scale-110 transition-transform">⭐</button>
            ))}
          </div>
        </div>
      </div>
...
function ScrollButtons({ containerRef, color = "brand" }) {
  ...
  const bgColor = color === "brand" ? "bg-brand-500/80 hover:bg-brand-500" : "bg-blue-500/80 hover:bg-blue-500";
...
export default function DriverPanel() {
  const {
    ride, driverPosition, navStats,
    acceptRide, startRide, completeRide, rejectRide, clearRide,
    subscribeToPosition, getCurrentPosition,
    nearbyRides, getFormattedDistance,
      ...
      >
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          ...
          <div>
            ...
            <p className="text-dark-400 text-xs mt-0.5">Accept rides within {MAX_DRIVER_DISTANCE_KM}km radius</p>
          </div>
          <div className="ml-auto">
            <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${
              hasRide ? "bg-green-500/20 text-green-400" : "bg-dark-700 text-dark-400"
            }`}>
              <span className={`w-1.5 h-1.5 rounded-full ${hasRide ? "bg-green-400 animate-ping-slow" : "bg-dark-500"}`} />
        ...
        <div className="w-full pb-4">
          ...
          {!ride && (
            <div className="flex flex-col gap-4 w-full">
              ...
              {hasNearbyRides && (
                <div className="space-y-2">
                  <p className="text-dark-400 text-[10px] uppercase tracking-wider px-1">Nearby Requests</p>
                  {nearbyRides.map((nearbyRide, idx) => (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <span className="text-brand-400 font-semibold text-sm">NPR {nearbyRide.fare}</span>
                      </div>
          ...
          {isComplete && (
            <div
              className="fixed inset-0 flex items-end lg:items-center justify-center z-50 pointer-events-auto p-4"
              style={{ background: "rgba(0,0,0,0.7)" }}
              onClick={(e) => e.target === e.currentTarget && clearRide()}
            >
              <div className="w-full max-w-sm bg-dark-900 rounded-3xl p-0 max-h-[80vh] overflow-y-auto">
                <PaymentScreen ride={ride} onClear={clearRide} />
              </div>
            </div>
          )}
          ...
          {isTerminal && !isComplete && (
            <div className="space-y-3 w-full">
              <div className={`rounded-xl p-4 border text-sm ${
                ride.status === RIDE_STATUS.REJECTED  ? "bg-red-500/10 border-red-500/20 text-red-300" :
                ride.status === RIDE_STATUS.CANCELLED ? "bg-dark-700/50 border-dark-600 text-dark-300" :
          ...
          {isPending && (
            ...
            >
              ...
              <div className="bg-dark-800/50 border border-dark-700 rounded-xl p-4 space-y-3 w-full">
                <p className="text-xs font-medium text-dark-400 uppercase tracking-wider">Trip Details</p>
                <div className="space-y-2">
                  {[
                    { icon: MapPin, color: "text-brand-400", bg: "bg-brand-500/20", label: "Pickup",
                      lat: ride.pickup.lat, lng: ride.pickup.lng },
                    { icon: Navigation, color: "text-green-400", bg: "bg-green-500/20", label: "Dropoff",
            ...
            <div className="space-y-3 w-full">
              ...
              <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4">
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-9 h-9 bg-blue-500/20 rounded-xl flex items-center justify-center text-xl">🚗</div>
                  <div>
                ...
                <NavStrip navStats={navStats} phase="to_pickup" />
              </div>
              ...
              {navStats && navStats.remainingM > 0 && (
                <div className="bg-dark-800/50 rounded-xl p-3">
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-brand-400 font-bold">
                      {formatTime(navStats.remainingS)}
                    </span>
                  </div>
          ...
          {isActive && (
            <div className="space-y-3 w-full">
              ...
              <div className="bg-brand-500/10 border border-brand-500/20 rounded-xl p-4">
                ...
                <NavStrip navStats={navStats} phase="to_dropoff" />
              </div>
              ...
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
                ...
                <CheckCircle size={15} />Complete Ride
              </motion.button>
            </div>
          )}
        </div>
