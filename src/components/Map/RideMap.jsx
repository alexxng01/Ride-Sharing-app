  ...
  const trySetView = useCallback(() => {
    ...
    if (!isMapPanesReady(map)) {
      ...
      retryRef.current = setTimeout(() => {
        if (mountedRef.current) trySetView();
      }, 500); // Increased from 150ms to 500ms
      return;
    }
    safeSetView(map, center, zoom, { animate: true }); // Changed to animate: true for smoother transitions
...
// ── ETA Display Overlay ───────────────────────────────────────────────────────
// UPDATED: Removed auto-updating timer, now shows static ETA
const ETADisplayOverlay = memo(function ETADisplayOverlay({
  initialEtaSeconds, distanceKm, phase, isLive
}) {
  // REMOVED: The useState and useEffect that was updating every 500ms
  // Now just displays the initial ETA without auto-refresh
  
  const minutes = Math.floor(initialEtaSeconds / 60);
  const seconds = Math.floor(initialEtaSeconds % 60);
    ...
    <div className="absolute bottom-20 left-3 right-3 z-[999] pointer-events-none">
      <div className="bg-dark-900/95 backdrop-blur-md border border-brand-500/30 rounded-2xl px-4 py-3 shadow-2xl">
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="text-right">
            ...
            <p className="text-white text-sm font-medium">
              {new Date(Date.now() + initialEtaSeconds * 1000).toLocaleTimeString([], {
        ...
        <div className="w-full h-1.5 bg-dark-700 rounded-full overflow-hidden">
          <div
            className="h-full rounded-full transition-all duration-1000" // Slower transition
            style={{
              width: '50%', // Static progress instead of animated
              background:
                phase === "to_pickup"
        ...
        <div className="flex items-center justify-between mt-2 text-[10px] text-dark-500">
          <span>Estimated ETA: {fmtTime(initialEtaSeconds)}</span>
          <span>ETA: {fmtTime(initialEtaSeconds)}</span>
        </div>
      </div>
    </div>
  );
...
function RideMap({ onLocationSelect, selectionMode }) {
  ...
  // UPDATED: Throttle driver position updates to every 5 seconds instead of real-time
  ...
  // Subscribe with throttling - update at most every 5 seconds
  useEffect(() => {
    ...
    unsubscribeRef.current = subscribe((position) => {
      if (position && !isNaN(position.lat) && !isNaN(position.lng)) {
        ...
        if (now - lastUpdateTime >= 5000) {
          ...
          // Store the latest position but don't update state yet
  ...
  }, [lastUpdateTime]); // Only re-run when lastUpdateTime changes
  ...
  // Calculate ETA - now only updates when ride status changes or driver moves significantly
  const etaData = (() => {
  ...
  return (
    <div className="relative w-full h-full rounded-2xl overflow-hidden shadow-2xl ring-1 ring-white/10">
      ...
      {/* ── Live ETA overlay (NO AUTO-REFRESH) ────────────────────── */}
      {isLivePhase && etaData && etaData.seconds > 0 && (
        <ETADisplayOverlay
          initialEtaSeconds={etaData.seconds}
